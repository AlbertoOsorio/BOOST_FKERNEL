...
function _obj_func!(fieldmap, B, by_min, by_max, grad_rms, Gx, Gy, Gz, d,   # Valores base y alocaciones
    ...
    _grad_fused!(Gx, Gy, Gz, B, d, nx, ny, nz, idx, i, j, k, N)
    ...
    tid = threadIdx().x
    _metrics!(B, by_min, by_max, grad_rms, shared_min, shared_max, shared_sum, idx, tid, N)

    return # Es un kernel mutable, no debemos retornar nada
end

# Determina el indice 1D de acceso al array
@inline function _idx(i,j,k, nx,ny)
...
@inline function _grad_fused!(Gx, Gy, Gz, B, d, nx, ny, nz, idx, i, j, k, N)
    
    #grad y
    if 1 < i < nx
        Gx[idx] = (B[_idx(i+1,j,k,nx,ny)] - B[_idx(i-1,j,k,nx,ny)]) / (2f0*d)
    elseif i == 1
        Gx[idx] = (B[_idx(2,j,k,nx,ny)] - B[_idx(1,j,k,nx,ny)]) / d
    else
        Gx[idx] = (B[_idx(nx,j,k,nx,ny)] - B[_idx(nx-1,j,k,nx,ny)]) / d
    end

    #grad y
    if 1 < j < ny
        Gy[idx] = (B[_idx(i,j+1,k,nx,ny)] - B[_idx(i,j-1,k,nx,ny)]) / (2f0*d)
    elseif j == 1
        Gy[idx] = (B[_idx(i,2,k,nx,ny)]   - B[_idx(i,1,k,nx,ny)])   / d
    else
        Gy[idx] = (B[_idx(i,ny,k,nx,ny)]  - B[_idx(i,ny-1,k,nx,ny)]) / d
    end

    #grad z
    if 1 < k < nz
        Gz[idx] = (B[_idx(i,j,k+1,nx,ny)] - B[_idx(i,j,k-1,nx,ny)]) / (2f0*d)
    elseif k == 1
        Gz[idx] = (B[_idx(i,j,2,nx,ny)] - B[_idx(i,j,1,nx,ny)]) / d
    else
        Gz[idx] = (B[_idx(i,j,nz,nx,ny)] - B[_idx(i,j,nz-1,nx,ny)]) / d
    end
    sync_threads()
...
@inline function _metrics!(B, global_min, global_max, global_sum_sq, shared_min, shared_max, shared_sum,
                    idx, tid, N)
